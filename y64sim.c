...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    int b = (argB >> 63) & 0x1;
    int v = (val >> 63) & 0x1;
    //zero and sign is right already then we judge overflow
    switch (op) {
        case A_ADD:
            if ((!a && !b && v) || (a && b && !v))
                ovf = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    //init doit = FALSE; then we set true case
    switch (cond) {
        case C_YES:
            doit = TRUE;
            break;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    reg_a = GET_REGA(codefun);
    reg_b = GET_REGB(codefun);
    val_a = get_reg_val(sim->r, reg_a);
    val_b = get_reg_val(sim->r, reg_b);
    next_pc++;
...
//        I_ALU, I_JMP, I_CALL, I_RET, I_PUSHQ, I_POPQ, I_DIRECTIVE } itype_t;
    ...
    if (icode == I_IRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ ||
        ...
        if (!get_long_val(sim->m, next_pc, &immNum)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        next_pc += 8;
    ...
    switch (icode) {
      case I_HALT: /* 0:0 */
        ...
        case I_IRMOVQ: /* 3:0 F:regB imm */
            set_reg_val(sim->r, GET_REGB(codefun), immNum);
            sim->pc = next_pc;
            break;
        case I_RMMOVQ: /* 4:0 regA:regB imm */
            if(!set_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + immNum, get_reg_val(sim->r, GET_REGA(codefun)))){
        ...
        case I_MRMOVQ: /* 5:0 regB:regA imm */
            if (!get_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + immNum, &temp)) {
                err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,
                          get_reg_val(sim->r, GET_REGB(codefun)) + immNum);
                return STAT_ADR;
            }
            set_reg_val(sim->r, GET_REGA(codefun), temp);
            sim->pc = next_pc;
            break;
        case I_ALU: /* 6:x regA:regB */
            temp = compute_alu(ifun, get_reg_val(sim->r, GET_REGA(codefun)), get_reg_val(sim->r, GET_REGB(codefun)));
            set_reg_val(sim->r, GET_REGB(codefun), temp);
            sim->cc = compute_cc(ifun, get_reg_val(sim->r, GET_REGA(codefun)), get_reg_val(sim->r, GET_REGB(codefun)),temp);
            sim->pc = next_pc;
            break;
        case I_JMP: /* 7:x imm */
            if(cond_doit(sim->cc, ifun) && immNum <= sim->m->len) sim->pc = immNum;
            else sim->pc = next_pc;
            break;
        case I_CALL: /* 8:x imm */
            ...
            break;
        case I_RET: /* 9:0 */
            get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &temp);
            set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) + 8);
            sim->pc = temp;
            break;
        case I_PUSHQ: /* A:0 regA:F */
            temp = get_reg_val(sim->r, GET_REGA(codefun));
            set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) - 8);
